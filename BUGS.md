...
| BUG-18 | High | After a mistake, continuing advanced the counter but froze the card | v111 — an Again-retry that resurfaced immediately kept the same React key (`rid`), so the drill never remounted. Cards now keyed `rid#attempt` (bumped per Again in `trackRetry`); `handleDrillGrade` also guards double-grading one attempt. Reported live by owner. |
