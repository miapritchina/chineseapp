...
| Supabase `bug_reports` | In-app bug reports (v144): free-text `note` + auto-captured `context` JSONB (page, active word/char, version, viewport, UA, timestamp — `lib/bugReport.ts`). Append-only from the client; INSERT allowed to **anon and authenticated** (the one anon-writable user table — a report is worth catching even unsigned), SELECT owner-only (anon key **can't read** — needs service role / dashboard). Triage runbook: [bug-report-triage.md](bug-report-triage.md) | Live; write-only, `useBugReport` + `BugReportModal` |
