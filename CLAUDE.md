...
>
> Known gap to close (pre-dates this directive): `useSaved`, `useReview`,
> `useMnemonics` currently treat `localStorage` as authoritative when
> signed out, and the Sentence Studio (`useSentenceDraft`,
> `useSavedSentences`) has **no** DB table at all (`e2.draft` /
> `e2.sentences` are local-only). Bringing these in line means a
> `user_sentences` migration + reworking those hooks so the DB is the
> source of truth.
...
│   │   ├── useSentenceDraft         sentence composer state (local-only today — owes a DB table; see Data persistence policy)
│   │   ├── useSavedSentences        saved sentences list (local-only today — owes a DB table)
...
- **User-private state** (saved words, status, FSRS scheduler,
  mnemonics, and — once the migration lands — sentences) lives in
  Supabase tables `user_saves`, `user_fsrs_state`, `user_mnemonics`
  (and a planned `user_sentences`). RLS policies restrict to
  `auth.uid() = user_id`. **Supabase is the source of truth** (see the
  Data persistence policy at the top); `localStorage` is only an
  offline read-cache.
